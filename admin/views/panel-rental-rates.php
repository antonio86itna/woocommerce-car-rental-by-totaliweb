...
<div id="rental_rates_data" class="panel woocommerce_options_panel show_if_rental_vehicle hidden">
    <div class="options_group">
        ...
        woocommerce_wp_text_input( array(
            'id'          => '_wcrbtw_base_daily_rate',
            'label'       => sprintf( __( 'Base Daily Rate (%s)', 'woocommerce-car-rental' ), $currency ),
            'type'        => 'text',
            'class'       => 'wc_input_price',
            'value'       => wc_format_localized_price( $base_daily_rate ?: '' ),
            'data_type'   => 'price',
            'desc_tip'    => true,
        ...
        <div id="rental_seasonal_rates_container" class="wcrbtw-repeater-container">
            <table class="widefat wcrbtw-repeater-table">
                ...
                <tbody id="rental_seasonal_rates_tbody">
                    <?php
                    // Seasonal rates already loaded from meta
                    ...
                    <tr class="wcrbtw-repeater-row">
                        <td>
                            <input type="text" 
                                   name="_wcrbtw_seasonal_rates[<?php echo esc_attr( $index ); ?>][name]" 
                                   value="<?php echo esc_attr( $rate['name'] ?? '' ); ?>" 
                                   placeholder="<?php esc_attr_e( 'e.g., Summer Season', 'woocommerce-car-rental' ); ?>" />
                        </td>
                        <td>
                            <input type="date" 
                                   name="_wcrbtw_seasonal_rates[<?php echo esc_attr( $index ); ?>][start_date]" 
                                   value="<?php echo esc_attr( $rate['start_date'] ?? '' ); ?>" />
                        </td>
                        <td>
                            <input type="date" 
                                   name="_wcrbtw_seasonal_rates[<?php echo esc_attr( $index ); ?>][end_date]" 
                                   value="<?php echo esc_attr( $rate['end_date'] ?? '' ); ?>" />
                        </td>
                        <td>
                            <input type="text" 
                                   class="wc_input_price" 
                                   name="_wcrbtw_seasonal_rates[<?php echo esc_attr( $index ); ?>][rate]" 
                                   value="<?php echo esc_attr( wc_format_localized_price( $rate['rate'] ?? '' ) ); ?>" 
                                   placeholder="<?php echo esc_attr( $currency ); ?>" />
                        </td>
                        <td>
                            <input type="number" 
                                   name="_wcrbtw_seasonal_rates[<?php echo esc_attr( $index ); ?>][priority]" 
                        ...
                        <td>
                            <input type="checkbox" 
                                   name="_wcrbtw_seasonal_rates[<?php echo esc_attr( $index ); ?>][recurring]" 
                                   value="1" 
...
<script type="text/template" id="tmpl-seasonal-rate-row">
    <tr class="wcrbtw-repeater-row">
        <td>
            <input type="text" 
                   name="_wcrbtw_seasonal_rates[{{data.index}}][name]" 
                   placeholder="<?php esc_attr_e( 'e.g., Summer Season', 'woocommerce-car-rental' ); ?>" />
        </td>
        <td>
            <input type="date" 
                   name="_wcrbtw_seasonal_rates[{{data.index}}][start_date]" />
        </td>
        <td>
            <input type="date" 
                   name="_wcrbtw_seasonal_rates[{{data.index}}][end_date]" />
        </td>
        <td>
            <input type="text" 
                   class="wc_input_price" 
                   name="_wcrbtw_seasonal_rates[{{data.index}}][rate]" 
                   placeholder="<?php echo esc_attr( $currency ); ?>" />
        </td>
        <td>
            <input type="number" 
                   name="_wcrbtw_seasonal_rates[{{data.index}}][priority]" 
                   value="0" 
                   min="0" 
                   step="1" 
                   style="width: 60px;" />
        ...
        <td>
            <input type="checkbox" 
                   name="_wcrbtw_seasonal_rates[{{data.index}}][recurring]" 
                   value="1" 
                   title="<?php esc_attr_e( 'Repeat annually', 'woocommerce-car-rental' ); ?>" />
        </td>
        <td>
